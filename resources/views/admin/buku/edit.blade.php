...
@section('title', 'Edit Buku — Synister Library')
    ...
    <div class="row g-3">
                ...
                <div class="cd-body">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <select name="category_id" class="fctrl">
                                ...
                                @foreach($kategori as $k)
                                    <option value="{{ $k->id }}" {{ old('category_id', $buku->category_id) == $k->id ? 'selected' : '' }}>
                                        {{ $k->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="deskripsi" class="fctrl" rows="4">{{ old('deskripsi', $buku->deskripsi) }}</textarea>
        ...
        <div class="col-12 col-lg-4">
            <div class="card-d">
                ...
                <div class="cd-body">
                    <div style="width:100%;aspect-ratio:2/3;background:var(--navy3);border-radius:10px;display:flex;align-items:center;justify-content:center;margin-bottom:14px;overflow:hidden;border:1px solid rgba(201,168,76,.15);">
                        @if($buku->cover_image)
                            <img id="cover-img" src="{{ Storage::url($buku->cover_image) }}" style="width:100%;height:100%;object-fit:cover;">
                        @else
                    ...
                    <label for="cover_image" class="btn-oA w-100 justify-content-center" style="cursor:pointer;">
                        <i class="bi bi-upload"></i> {{ $buku->cover_image ? 'Ganti Cover' : 'Pilih Gambar' }}
                    </label>
                    <input type="file" id="cover_image" name="cover_image" accept="image/jpg,image/jpeg,image/png" style="display:none;" onchange="previewCover(this)">
                    <div style="font-size:.72rem;color:var(--muted);margin-top:8px;text-align:center;">Kosongkan jika tidak ingin mengganti</div>
                    @error('cover_image')<div class="ferr">{{ $message }}</div>@enderror
                </div>
            </div>
            <div class="d-flex flex-column gap-2 mt-3">
                <button type="submit" class="btn-A w-100 justify-content-center"><i class="bi bi-check-lg"></i> Simpan Perubahan</button>
...
function previewCover(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = e => {
            ...
            let ph = document.getElementById('cover-placeholder');
